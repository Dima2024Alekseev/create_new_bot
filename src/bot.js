...
const { 
  ...
  extendSubscription, 
  promptForQuestion, 
  requestVpnInfo,       // Теперь импортируются из userController
  handleVpnConfigured,  // Теперь импортируются из userController
  promptVpnFailure,     // Теперь импортируются из userController
  promptCancelSubscription, 
  cancelSubscriptionFinal, 
  cancelSubscriptionAbort,
  // Добавим эти, если они есть в userController
  help, 
  faq, 
  contactAdmin 
...
bot.command('status', checkSubscriptionStatus); // Используем checkSubscriptionStatus для /status
bot.command('faq', faq); 
bot.command('contact', contactAdmin); 
...
bot.action(/send_vpn_info_(\d+)/, requestVpnInfo); // Используем из userController
bot.action(/vpn_configured_(\d+)/, handleVpnConfigured); // Используем из userController
bot.action(/vpn_failed_(\d+)/, promptVpnFailure); // Используем из userController
