require('dotenv').config({ path: __dirname + '/../primer.env' });
   ...
   const mongoose = require('mongoose'); // Добавляем mongoose для доступа к connection
   const connectDB = require('./config/db');
   const User = require('./models/User');

   // Логирование версий зависимостей
   console.log('Версия telegraf-session-mongodb:', require('telegraf-session-mongodb/package.json').version);
   console.log('Версия mongoose:', require('mongoose/package.json').version);
   console.log('Версия mongodb:', require('mongodb/package.json').version);
   ...
   const setupSession = async () => {
     ...
     try {
       await connectDB();
       console.log('MongoDB подключена успешно, состояние:', mongoose.connection.readyState);
       console.log('Имя базы данных:', mongoose.connection.name);
       console.log('Доступные коллекции:', await mongoose.connection.db.listCollections().toArray());

       // Используем mongoose.connection вместо прямого url
       if (typeof sessionMongo.session === 'function') {
         console.log('sessionMongo.session является функцией, используем mongoose.connection');
         bot.use(sessionMongo.session({
           database: mongoose.connection, // Передаём mongoose подключение
           collectionName: 'users',
           sessionField: 'session',
           model: User
         }));
       } else {
         console.error('Ошибка: sessionMongo.session не является функцией:', sessionMongo);
   ...
   // Вспомогательные функции для изменения цены и реквизитов
