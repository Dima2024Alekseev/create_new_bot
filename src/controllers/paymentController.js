...
/**
 * Отклонение платежа без комментария (дефолтная причина)
 */
exports.handleReject = async (ctx, userIdParam) => {
    const userId = parseInt(userIdParam || ctx.match[1]);
    try {
        await User.findOneAndUpdate(
            { userId },
            {
...
/**
 * Отклонение платежа с комментарием
 */
exports.handleRejectWithComment = async (ctx, userId, comment) => {
    try {
        await User.findOneAndUpdate(
            { userId },
            {
        ...
        await ctx.reply('Комментарий отправлен пользователю, платёж отклонён.');
    } catch (error) {
        console.error(`Ошибка при отклонении платежа с комментарием для пользователя ${userId}:`, error);
        await ctx.reply('⚠️ Произошла ошибка при отклонении платежа. Проверьте логи.');
    }
};

/**
 * Отмена отклонения платежа (возврат в pending)
 */
exports.cancelReject = async (ctx, userId) => {
    try {
        await User.findOneAndUpdate(
            { userId },
            { status: 'pending' }
        );
        await ctx.editMessageText('Отклонение отменено, заявка возвращена на рассмотрение.');
    } catch (error) {
        console.error(`Ошибка при отмене отклонения платежа для пользователя ${userId}:`, error);
        await ctx.reply('⚠️ Не удалось отменить отклонение платежа.');
    }
};