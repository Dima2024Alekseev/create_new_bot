...
const cleanInactiveUsers = async (bot) => {
  try {
    ...
    console.log(`[Cron][${now.toLocaleString('ru-RU', { timeZone: 'Asia/Krasnoyarsk' })}] Найдено ${inactiveUsers.length} неактивных пользователей, ${expiredSubscriptionUsers.length} пользователей с истёкшей подпиской и ${cancelledSubscriptionUsers.length} пользователей с отменённой подпиской для удаления.`);
    ...
    // Удаление пользователей, отменивших подписку
    for (const user of cancelledSubscriptionUsers) {
      try {
        if (user.vpnClientName) {
          await deleteVpnClient(user.vpnClientName);
          console.log(`[DEBUG] Удалён VPN-клиент: ${user.vpnClientName} для ${user.userId}`);
        ...
        await User.deleteOne({ userId: user.userId });
        console.log(`[DEBUG] Удалён пользователь с отменённой подпиской: ${user.userId}`);

        await bot.telegram.sendMessage(
          process.env.ADMIN_ID,
          `🗑 *Удалён пользователь с отменённой подпиской:* ${user.firstName || user.username} (ID: ${user.userId})`,
          { parse_mode: 'Markdown' }
        );
      } catch (e) {
        console.error(`Ошибка удаления пользователя с отменённой подпиской ${user.userId}:`, e);
      }
    }
  } catch (err) {
