...
const {
  handleStart,
  // checkSubscriptionStatus, // Эту функцию мы теперь не будем вызывать напрямую из кнопок
  extendSubscription,
  promptForQuestion,
  requestVpnInfo,
  handleVpnConfigured,
  ...
  showMySubscription // <--- ИМПОРТИРУЕМ НОВУЮ ФУНКЦИЮ
...
bot.on('text', async (ctx, next) => {
  ...
  if (!ctx.message.text.startsWith('/')) {
    ...
    // Если у пользователя статус inactive или rejected И он не админ,
    // то его текст расценивается как проблема с отправкой скрина или вопрос.
    // Если он уже "pending", то ожидаем, пока админ проверит скриншот.
    if (user && (user.status === 'inactive' || user.status === 'rejected') && !ctx.session?.awaitingAnswerFor) {
      // Проверка, что сообщение не пустое и не просто пробелы
      if (ctx.message.text.trim().length > 0) {
        // Если пользователь ввел текст, когда у него нет активной подписки (или она отклонна)
        // это, вероятно, вопрос к админу. Обрабатываем как вопрос.
        await handleQuestion(ctx);
        return;
      } else {
        // Пустое сообщение, напоминаем про скриншот
        await ctx.reply(
      ...
      // Если пользователь "pending" и что-то пишет, это скорее всего вопрос.
      await handleQuestion(ctx);
      return;
    } else if (!user) {
      // Новый пользователь, который пишет текст до /start.
      // Можем перенаправить его на /start или обработать как вопрос.
      // Сейчас просто показываем начальное сообщение.
      await handleStart(ctx);
      return;
    }
    // Если пользователь активен и не находится в состоянии ожидания ответа на проблему VPN,
    // то любой другой текст расценивается как вопрос.
    await handleQuestion(ctx);
    return;
  } else {
    // Это команда, передаем дальше
    return next();
  }
});
...
// Кнопки пользователя
bot.action('my_subscription', showMySubscription); // <--- НОВЫЙ ОБРАБОТЧИК
// bot.action('check_subscription', checkSubscriptionStatus); // УДАЛЕНО/ЗАКОММЕНТИРОВАННО - функционал перенесен в 'my_subscription'
