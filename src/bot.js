...
const { setConfig, getConfig, getPaymentDetailsConfig, setPaymentDetailsConfig } = require('./services/configService');
...
async function finalizePaymentDetailsChange(ctx) {
    const { sbp_phone, card_number, payment_comment } = ctx.session.pendingPaymentDetails;
    const oldDetails = await getPaymentDetailsConfig();

    try {
        await setPaymentDetailsConfig({
            sbp_phone: sbp_phone || oldDetails.sbp_phone,
            card_number: card_number || oldDetails.card_number,
            payment_comment: payment_comment || oldDetails.payment_comment
        });

        delete ctx.session.awaitingPaymentDetails;
        delete ctx.session.pendingPaymentDetails;

        await ctx.reply(
            `✅ Реквизиты успешно обновлены:\n` +
            `📱 СБП: \`${sbp_phone || oldDetails.sbp_phone}\`\n` +
            `💳 Карта: \`${card_number || oldDetails.card_number}\`\n` +
            `📝 Комментарий: \`${payment_comment || oldDetails.payment_comment}\``,
            { parse_mode: 'Markdown' }
        );
        await checkAdminMenu(ctx);

        console.log(`[PAYMENT DETAILS CHANGE] Admin ${ctx.from.id} updated payment details`);
    } catch (error) {
        console.error('Ошибка при сохранении новых реквизитов:', error);
    ...
    if (ctx.from?.id === parseInt(process.env.ADMIN_ID)) {
        ...
        if (ctx.session?.awaitingPaymentDetails && ctx.message?.text) {
            ...
            for (const line of lines) {
                ...
                if (key && value) {
                    if (key.includes('СБП')) {
                        details.sbp_phone = value === '-' ? null : value;
                        if (value !== '-' && !/^\+\d{1,2}\s*\(\d{3}\)\s*\d{3}-\d{2}-\d{2}$/.test(value)) {
                            isValid = false;
                            await ctx.reply('❌ Номер телефона СБП должен быть в формате +7 (999) 123-45-67');
                        }
                    } else if (key.includes('Карта')) {
                        details.card_number = value === '-' ? null : value;
                        if (value !== '-' && !/^\d{4}\s*\d{4}\s*\d{4}\s*\d{4}$/.test(value)) {
                            isValid = false;
                            await ctx.reply('❌ Номер карты должен быть в формате 1234 5678 9012 3456');
                        }
                    } else if (key.includes('Комментарий')) {
                        details.payment_comment = value === '-' ? null : value;
                        if (value !== '-' && (!value.includes('{userId}') || value.length > 100)) {
                            isValid = false;
                            await ctx.reply('❌ Комментарий должен содержать {userId} и быть не длиннее 100 символов');
                        }
                    }
                }
            }

            if (!isValid || Object.keys(details).length < 3) {
                return ctx.reply(
                    '⚠️ Проверьте формат ввода. Пример:\n' +
                    'СБП: +7 (999) 123-45-67\n' +
            ...
            const oldDetails = await getPaymentDetailsConfig();
            ctx.session.pendingPaymentDetails = details;

            await ctx.reply(
                `⚠️ Подтвердите изменение реквизитов:\n` +
                `📱 СБП: \`${details.sbp_phone || oldDetails.sbp_phone}\`\n` +
                `💳 Карта: \`${details.card_number || oldDetails.card_number}\`\n` +
                `📝 Комментарий: \`${details.payment_comment || oldDetails.payment_comment}\`\n\n` +
                `Подтвердите:`,
                {
                    parse_mode: 'Markdown',
                    reply_markup: Markup.inlineKeyboard([
                        [Markup.button.callback('✅ Подтвердить', 'confirm_payment_details')],
                        [Markup.button.callback('❌ Отмена', 'cancel_payment_details')]
                    ])
                }
            );
...
bot.on('text', async (ctx, next) => {
    if (ctx.from?.id === parseInt(process.env.ADMIN_ID) && (
        ...
        ctx.session?.awaitingPaymentDetails
    )) {
        return next();
    }
...
bot.action('set_payment_details_admin', setPaymentDetails);

bot.action('confirm_payment_details', async (ctx) => {
    if (!checkAdmin(ctx)) return ctx.answerCbQuery('🚫 Только для админа');

    await finalizePaymentDetailsChange(ctx);
    await ctx.answerCbQuery();
});

bot.action('cancel_payment_details', async (ctx) => {
    if (!checkAdmin(ctx)) return ctx.answerCbQuery('🚫 Только для админа');
