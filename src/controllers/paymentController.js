...
const { formatDate, escapeMarkdown, transliterate, generateRejectionMessage } = require('../utils/helpers');
...
class PaymentController {
  ...
  async handleReject(ctx) {
    ...
    try {
      const user = await User.findOne({ userId });
      const paymentPhotoId = user.paymentPhotoId;
        ...
        `❌ Вы отклонили платёж пользователя ${userId}.`,
        {
          reply_markup: Markup.inlineKeyboard([
            [
              ...
              Markup.button.callback('↩️ Вернуть на проверку', `undo_reject_${userId}`)
            ],
            [
              Markup.button.callback('❌ Отклонить без комментария', `confirm_reject_${userId}`)
            ]
          ]).reply_markup
        }
      );
   ...
   */
  async handleAddRejectComment(ctx) {
    const userId = parseInt(ctx.match[1]);
  ...
  /**
   * Обработчик кнопки "Вернуть на проверку"
   */
  async handleUndoReject(ctx) {
    const userId = parseInt(ctx.match[1]);
    ...
    try {
      ...
      // Отправляем скриншот обратно администратору
      await ctx.telegram.sendPhoto(
        ctx.chat.id,
        user.paymentPhotoId,
        {
          caption: `🔄 Платёж пользователя ${userId} возвращён на проверку`,
          reply_markup: Markup.inlineKeyboard([
            [
      ...
      await ctx.answerCbQuery('✅ Платёж возвращён на проверку');
      await ctx.deleteMessage();
    } catch (error) {
      console.error('Ошибка при отмене отклонения:', error);
      await ctx.answerCbQuery('⚠️ Ошибка!');
    }
  }

  /**
   * Обработчик кнопки "Отклонить без комментария"
   */
  async handleConfirmReject(ctx) {
    const userId = parseInt(ctx.match[1]);
    ...
    try {
      ...
      await this.sendRejectionToUser(userId);
      await ctx.answerCbQuery('✅ Платёж отклонён');
      await ctx.deleteMessage();
    } catch (error) {
      console.error('Ошибка при подтверждении отклонения:', error);
      await ctx.answerCbQuery('⚠️ Ошибка!');
    }
  }
   ...
   */
  async sendRejectionToUser(userId, customReason = null) {
    const user = await User.findOne({ userId });
    
    try {
      await bot.telegram.sendMessage(
        userId,
        generateRejectionMessage(customReason || user.rejectionReason),
        { parse_mode: 'Markdown' }
      );
    } catch (error) {
  ...
  /**
   * Обработчик текстового комментария при отклонении
   */
  async handleRejectComment(ctx) {
    if (!ctx.session?.awaitingRejectComment || !checkAdmin(ctx)) return;
    ...
    await this.sendRejectionToUser(userId, comment);
